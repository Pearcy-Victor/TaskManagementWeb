/**
 * Purpose:     The /tasks/edit/:id page.
 * Responsibility: Load the task by :id, pre-fill a Reactive Form, and
 *                call TaskService.update() on submit.
 * Interactions:
 *   - ProjectService.getAll() → populates the project <select>.
 *   - TaskService.getById(id) → loads the task.
 *   - TaskService.update(id, form.value) → PUT /api/tasks/{id}.
 *
 * Demonstrates `switchMap` chained from a route param to an HTTP call,
 * which is the canonical "load by id" pattern in Angular.
 */
...
@Component({
  ...
  styles: [`
    ...
    input, textarea, select { padding: var(--space-2) var(--space-3); border: 1px solid var(--color-border); border-radius: var(--radius); font-family: inherit; }
    .row { display: flex; gap: var(--space-3); align-items: end; }
    button { padding: var(--space-3) var(--space-4); background: var(--color-primary); color: white; border: 0; border-radius: var(--radius); font-weight: 600; }
...
export class TaskEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap
      .pipe(switchMap((params) => this.tasksSvc.getById(params.get('id')!)))
      .subscribe({
        ...
        error: (err) => this.errorMessage.set(err?.error?.message ?? 'Failed to load task.')
      });
  }
  ...
  onSubmit(): void {
    ...
    this.tasksSvc.update(id, this.form.getRawValue() as any)
      .subscribe({
        next: () => { this.loading.set(false); this.router.navigate(['/tasks']); },
        error: (err) => { this.loading.set(false); this.errorMessage.set(err?.error?.message ?? 'Failed to save changes.'); }
      });
  }
}
